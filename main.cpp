// DSA_Stuff.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
namespace algorithm {
	...
	static bool walk(const std::vector<std::string>& maze, const char wall, const point& curr, const point& end, std::vector<std::vector<bool>>& seen, std::vector<point>& path) {
		//out of bounds
	...
	static std::vector<point> solve_maze(const std::vector<std::string>& maze, const char wall, const point start, const point end) {
...
namespace util {
	...
	static void print_maze(const std::string& message, const std::vector<std::string>& maze) {
...
int main()
{
	...
	auto search = 4;
	std::cout << "The value " << search << (algorithm::binary_search(vec, search) ? " was " : " was not ") << "found" << std::endl;
